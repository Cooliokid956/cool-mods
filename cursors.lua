...
function rendercursors()
    ...
    gPlayerSyncTable[0].mouse.x = djui_hud_get_mouse_x()/screenwidth
    gPlayerSyncTable[0].mouse.y = djui_hud_get_mouse_y()/screenheight

    if gMarioStates[0].controller.buttonDown & (B_BUTTON | A_BUTTON) ~= 0 then
        djui_hud_render_texture_interpolated(handgrab,gPlayerSyncTable[0].mouse.prev.x*screenwidth,gPlayerSyncTable[0].mouse.prev.y*screenheight,unit()/400,unit()/400,gPlayerSyncTable[0].mouse.x*screenwidth,gPlayerSyncTable[0].mouse.y*screenheight,unit()/400,unit()/400)
        ...
        djui_hud_render_texture_interpolated(hand,gPlayerSyncTable[0].mouse.prev.x*screenwidth,gPlayerSyncTable[0].mouse.prev.y*screenheight,unit()/400,unit()/400,gPlayerSyncTable[0].mouse.x*screenwidth,gPlayerSyncTable[0].mouse.y*screenheight,unit()/400,unit()/400)
    end
    for i=1, #gNetworkPlayers, 1 do
        if gNetworkPlayers[i].connected and gPlayerSyncTable[i].mouse ~= nil then
            if gMarioStates[i].controller.buttonDown & (B_BUTTON | A_BUTTON) ~= 0 then
                djui_hud_render_texture_interpolated(handgrab,gPlayerSyncTable[i].mouse.prev.x*screenwidth,gPlayerSyncTable[i].mouse.prev.y*screenheight,unit()/400,unit()/400,gPlayerSyncTable[i].mouse.x*screenwidth,gPlayerSyncTable[i].mouse.y*screenheight,unit()/400,unit()/400)
                if gMarioStates[i].controller.buttonPressed & (B_BUTTON | A_BUTTON) ~= 0 and sound then
                    play_sound(SOUND_MENU_CLICK_FILE_SELECT,gMarioStates[0].marioObj.header.gfx.cameraToObject)
                end
            else
                djui_hud_render_texture_interpolated(hand,gPlayerSyncTable[i].mouse.prev.x*screenwidth,gPlayerSyncTable[i].mouse.prev.y*screenheight,unit()/400,unit()/400,gPlayerSyncTable[i].mouse.x*screenwidth,gPlayerSyncTable[i].mouse.y*screenheight,unit()/400,unit()/400)
            end
        end
    end
end
