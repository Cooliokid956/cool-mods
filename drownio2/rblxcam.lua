...
function capture_c_buttons(m)
    ...
    local offset = 150
    rblxCam.targetDist = math.max(rblxCam.targetDist - ((c & U_CBUTTONS ~= 0) and offset or 0) + ((c & D_CBUTTONS ~= 0) and offset or 0), 0)
end
...
function roblox_cam(m)
    ...
    local mouse = rblxCam.dist < 10 or buttons.buttonDown & R_TRIG ~= 0
