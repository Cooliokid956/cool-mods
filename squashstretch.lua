...
function init_squash(m)
    local s = gSquashStates[m.playerIndex]
    s.y = m.pos.y
    s.yVel = 0
    s.damping = DEFAULT_DAMPING
    s.tension = DEFAULT_TENSION
end

hook_event(HOOK_ON_LEVEL_INIT, function ()
    for i = 0, MAX_PLAYERS-1 do
        init_squash(gMarioStates[i])
    end
end)
...
hook_event(HOOK_MARIO_UPDATE, function (m)
    local s = gSquashStates[m.playerIndex]
    if not m.marioObj
    or is_player_in_local_area(m) == 0 then
        s.active = false
        return
    ...
    m.marioObj.header.gfx.scale.y = m.marioObj.header.gfx.scale.y * scaleY
end)
