...
hook_event(HOOK_ON_HUD_RENDER, function ()
    ...
    local alpha = math.abs(math.sin(get_global_timer()/10))*255
    local alphaP = math.abs(math.sin((get_global_timer()-1)/10))*255
    djui_chat_message_create('res '..((res==0) and 'djui' or 'n64'))

    djui_hud_set_resolution(res)
    djui_hud_set_color_interpolated(20, 20, 20, alphaP, 20, 20, 20, alpha)
    ...
    if c & L_JPAD ~= 0 then
        djui_hud_set_color_interpolated(200, 20, 20, alphaP, 200, 20, 20, alpha)
    ...
    if c & D_JPAD ~= 0 then
        djui_hud_set_color_interpolated(20, 200, 20, alphaP, 20, 200, 20, alpha)
    ...
    if c & R_JPAD ~= 0 then
        djui_hud_set_color_interpolated(20, 20, 200, alphaP, 20, 20, 200, alpha)
